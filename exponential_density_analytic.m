...
%mass having exponential density distribution
clear all
close all
...
rho=@(x,y) -500.*(2.32.*10^-5.*x+1.5.*10^-5.*y);
...
for i2=1:1:n2
    ...
    for i1=1:1:n1
        ...
        hs1=2*pi*G.*exp(abs(K).*z0).*exp(-abs(kappa).*delta1);
        hs2=2*pi*G.*exp(abs(K).*z0).*exp(-abs(kappa).*delta2);
        tongF1=0; tongF2=0;
        %Taylor series sum
        for im=1:30
              im=im-1;
              ss1=rho(XX1,YY1).*(deltah1).^(im); ss2=rho(XX1,YY1).*(deltah2).^(im);
              vv1=(sfft2(ss1./(factorial(im)),xi,eta,0.5,0.5));
              vv2=(sfft2(ss2./(factorial(im)),xi,eta,0.5,0.5));
               
             tongF1=tongF1+(((-abs(kappa)).^(im-1))).*vv1;
             tongF2=tongF2+(((-abs(kappa)).^(im-1))).*vv2;
        end
